...
server <- function(input, output, session) {
        ...
        output$forecast_ui = renderUI({
                input$nextBtn
                numericInput("num", label = "###", value = NULL)
        })
        ...
        output$displayBox = renderValueBox({
                valueBox(value = data_series()$Environmental_Impact[25], width = 8,
                         subtitle = h4("#######"),icon = icon("th"),
                         color = "orange"
                )
        })
        
        output$title = renderUI({
                h3(paste("###", rv$page, "##", NUM_PAGES))
        })
        ...
        series_plot = reactive({
                ...
                g.int <- ggplot(data = ds, 
                        ...
                        ylim(0,5000)+
                        #coord_cartesian(ylim = c(0, max(ds$sale, na.rm=TRUE))) + 
