...
server <- function(input, output, session) {
        ...
        output$promotionBox = renderValueBox({
                valueBox(value = data_series()$promo[25], width = 12,
                         subtitle = h4("Promotion type at period 25"),icon = icon("star"),
                         color = "purple"
                )
        })
        ...
        output$displayBox = renderValueBox({
                valueBox(value = data_series()$Promotion_Impact[25], width = 12,
                         subtitle = h4("Promotion Impact at period 25"),icon = icon("th"),
                         color = "orange"
                )
        })
        ...
        observeEvent(input$finish, {
                ...
                save(res, file=paste0("results/",name_id, ".RData"))
                ...
                gs_add_row(sheet, input = c(name_id, res$prediction))
        })
