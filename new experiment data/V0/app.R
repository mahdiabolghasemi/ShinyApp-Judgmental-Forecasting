...
server <- function(input, output, session) {
        ...
        output$promotionBox = renderValueBox({
                valueBox(value = data_series()$promo[25], width = 12,
                         subtitle = h4("Promotion presence in the forecast period"),icon = icon("star"),
                         color = "purple"
                )
        })
        
        output$displayBox = renderValueBox({
                valueBox(value = data_series()$Environmental_Impact[25], width = 12,
                         subtitle = h4("Environmental Impact in the forecast period"),icon = icon("th"),
                         color = "orange"
                )
        })
        ...
        series_plot = reactive({
                ...
                g.int <- ggplot(data = ds, 
                        ...
                        geom_label_repel(point.padding = 1, nudge_x = -2.5,
                                         nudge_y = max(ds$sale*0.28, na.rm=TRUE),
                                         force = 2, colour = "black") + 
                        geom_point() + ylim(0,5000)+
                        geom_line() +
